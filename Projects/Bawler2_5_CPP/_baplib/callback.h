...
protected:
  ...
  CBFunctor0(Thunk t,const void* c,const void* f,size_t sz) //MZ Potentially some way to solve the problem with invalid conversions deeper down.
...
public:
  CBFunctionTranslator0(Func f):CBFunctor0(thunk,0,(void*)f,0){}//MZ adding a (void*) cast to f here clears cascading type errors.
  static void thunk(const CBFunctorBase &ftor)
    {
    (Func(ftor.func))();
    }
...
makeFunctor(CBFunctor0 *,TRT (*f)())
  {
  return CBFunctionTranslator0<TRT (*)()>(f); //MZ  potentially use a cast here to allow for pure C functions to work properly?
  }
